...
function findFirstSingleChar(str) {
  return str.split('').find((el, i, arr) => (arr.indexOf(el) === arr.lastIndexOf(el) ? el : 0));
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${leftBracket}${smallNum}, ${largeNum}${rightBracket}`;
}
...
function getDigitalRoot(num) {
  let sumOfDigits = num.toString().split('').reduce((acc, el) => acc + el, 0);
  while (sumOfDigits > 9) {
    sumOfDigits = (sumOfDigits % 10) + Math.floor(sumOfDigits / 10);
  }
  return sumOfDigits;
}
...
function getMatrixProduct(m1, m2) {
  const width = m1.length;
  const height = m2[0].length;

  const resultMatrix = Array(height)
    .fill(0).reduce((m) => {
      m.push(Array(width).fill(0));
      return m;
    }, []);
  ...
  for (let i = 0; i < m1.length; i += 1) {
    for (let j = 0; j < m2[0].length; j += 1) {
      let res = 0;
      for (let k = 0; k < m1[0].length; k += 1) res += m1[i][k] * m2[k][j];
